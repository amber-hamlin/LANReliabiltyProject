...
x=0:0.01:0.99; %probability range 
    for p = 0:0.01:0.99
        numberofTransmissions(index)= runTwoParallelLinkSim(K,p,N); %run simulation 
...
% label plot
semilogy(x, numberofTransmissions, markerColorMatrix(Kindex),'MarkerSize',8); %plot with log y-axis scale and linear x-axis 
hold on;
